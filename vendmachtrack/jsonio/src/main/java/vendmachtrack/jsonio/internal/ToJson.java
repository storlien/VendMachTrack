...
/**
 * The {@code ToJson} class provides methods to serialize a
 * {@link MachineTracker}
 * object into its JSON representation and write to either an OutputStream or a
 * file.
 * <p>
 * This class uses the Gson library to perform the serialization. The provided
 * file name
 * is used to determine the absolute path of the file in the user's home
 * directory, where
...
public class ToJson {
     ...
     * The full path is derived by appending the file name to the user's home
     * directory.
     * This path will be used to save the serialized JSON representation of the
    ...
    /**
     * Serializes the provided {@link MachineTracker} object into its JSON
     * representation
     * and returns the data as an {@link OutputStream}. The Gson library is used for
     * the serialization process.
     * <p>
     * This method is particularly useful for applications that might send
     * serialized data over
     * networks, for instance via TCP/IP protocol streams.
     * <p>
     * If there's an error during the serialization or writing process, an error is
     * printed
     ...
     * Serializes the provided {@link MachineTracker} object into its JSON
     * representation
     * and writes the resulting data to a file. The file's location is determined by
     * the file name
     * provided during the instantiation of this class.
     * <p>
     * The Gson library is used for the serialization process. If there are issues
     * during serialization
     * or writing the file, an error message will be printed to the standard error
     * stream.
     *
     * @param machtrack The {@link MachineTracker} object to be serialized and saved
     *                  to file.
     */
